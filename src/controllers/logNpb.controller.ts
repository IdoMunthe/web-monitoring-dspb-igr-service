...
export const getLogNpb = async (req: Request, res: Response) => {
  try {
    const { startDate, endDate, jenisNpb, statusKirim, kodeToko, kodeCabang } = req.query;

    if (!startDate || !endDate) {
      return res.status(400).json({
        message: "startDate and endDate (format: DD-MM-YYYY), jenisNpb, statusKirim, kodeToko, kodeCabang are required",
      });
    }
